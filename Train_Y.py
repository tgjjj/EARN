...
if __name__ == "__main__":
	...
	train_set = AR_Dataset.AR_Dataset(image_dir = './Flickr2K_Y', phase = "Train", 
                comp_quality = ["10", "20", "30", "40", "50", "60", "70", "80", "90"], 
                original_format = "png", crop_size = 256, samples_per_image = 4, channels = 1)
	train_data_loader = torch.utils.data.DataLoader(train_set, batch_size = 1, shuffle = True, num_workers = 1)
	val_set = AR_Dataset.AR_Dataset(image_dir = 'LIVE1_Y', phase = "Test", 
		comp_quality = ["20"], original_format = "png", crop_size = 512, channels = 1)
	val_data_loader = torch.utils.data.DataLoader(val_set, batch_size = 1, shuffle = True, num_workers = 1)

	model = Network.ARNet_3S_2N(in_out_plains = 1, base_plains = 64).cuda(device = 0)
	if args.multi_gpu:
